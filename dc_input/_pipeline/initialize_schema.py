...
from dc_input._types import UserInput, SessionStart, InputStep, KeyPath, SessionEnd, ContextStep, SchemaContainerShape
...
def initialize_schema(schema: type[T], inputs: list[UserInput]) -> T:
    ...
    ):  # Case: first graph_part after GraphStart is Node
        graph_start = graph_start.parent

    nodes: dict[KeyPath, ContextStep] = {}
    cur = graph_start.next
    while not isinstance(cur, SessionEnd):
        assert isinstance(cur, (InputStep, ContextStep))
        if isinstance(cur, ContextStep):
